...
public class Point implements Comparable<Point> {
  ...
  public static void main(String[] args) {
    ...
    Point d = new Point(15, 25);
    Point v = new Point(10, 10);
    Point h = new Point(80, 20);

    Point c = new Point(5, 20);

    StdOut.printf("Is same (0): %d\n", a.compareTo(s));
    StdOut.printf("Is >y (+1): %d\n", a.compareTo(v));
    StdOut.printf("Is <y <x (-1): %d\n", a.compareTo(d));
    StdOut.printf("Is =y <x (-1): %d\n", a.compareTo(h));
    StdOut.printf("Is =y >x (+1): %d\n", a.compareTo(c));

    StdOut.printf("Is same (-inf): %f\n", a.slopeTo(s));
    StdOut.printf("Is line (#): %f\n", a.slopeTo(d));
    StdOut.printf("Is vertical (inf): %f\n", a.slopeTo(v));
    StdOut.printf("Is horizontal (0): %f\n", a.slopeTo(h));

    Point[] arr = {a, s, d, v, h, c};

    Arrays.sort(arr, a.slopeOrder());

    for (Point p : arr) {
      StdOut.printf("%s ", p.toString());
    }
  }
}
